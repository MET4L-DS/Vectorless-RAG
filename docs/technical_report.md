...
2. **Intermediate Nodes (Level 1 - Chapters)**: Generated by performing **bottom-up roll-up summarization**. Leaf nodes are grouped, and the Google Gemini API (`models/gemini-3.1-flash-lite`) is invoked asynchronously to generate a concise summary of the Chapter's scope.
