...
4. **Cold-Start Resilience (Hugging Face Spaces)**: If the backend is hosted on a free Hugging Face Space, it goes to sleep after 48 hours of inactivity. Wrap initial load requests (`GET /api/chats/sessions` and `GET /api/chats/{thread_id}/history`) in client-side retry loops with exponential backoff (e.g. up to 5 retries starting at 2s) to handle container boot time transparently for the user.
