    ...
    try:
        ...
        async for chunk in agent.astream(
            ...
            if chunk_type == "custom":
                msg_text = chunk_data.get("message", "") if isinstance(chunk_data, dict) else str(chunk_data)
                if msg_text:
                ...
                for node, update in chunk_data.items():
                    if isinstance(update, dict) and "structured_response" in update and update["structured_response"]:
                        generated = update["structured_response"]
